...
**Phase 2 update, 2026-09-09:** R1 began with [R1a: saved processing presets](r1a-processing-presets.md). [R1b: portability, measurements and engine foundation](r1b-portability-measurements-and-engine.md) added portable preset/IR bundles, sample peak/RMS readings and a binary64 serial-rack prototype with a phone Kotlin/native comparison. [R1c: production effects and convolution](r1c-production-effects-and-convolution.md) ports existing processing to binary64. [R1d: decoder precision and Android output](r1d-decoder-precision-output.md) adds decoder-side processing with a single float32 output boundary. [R1e: saved serial racks, 64-band editing and portable app backups](r1e-serial-rack-and-backups.md) adds the persisted rack/editor, normal/crossfade/Studio global integration, V1 migration and app-backup IR assets. It also removes intermediate quantization between global nodes in the PCM16 compatibility path. Full hardware acceptance and remaining advanced routing/output work are still open; full R1 is not complete.

**Tuning update, 2026-09-09:** [R2a: strict EQ text interchange and calculated response](r2a-eq-import-response-and-phone-validation.md) adds file/paste import, explicit preamp + EQ append, parametric text export and magnitude/phase/group-delay views using the production coefficients. This pass also validates the current release on the connected Nothing A059P, fixes shuffle state across crossfade, and orders durable queue writes. The report records the actual device coverage and remaining limits; R2 remains in progress.

**Measurement tuning update, 2026-09-10:** [R2b: saved measurement projects and independent channel correction](r2b-measurement-tuning-and-phone-validation.md) adds a dedicated Measurement tuning dock, strict raw frequency-response imports, custom targets, saved offline projects, configurable magnitude fitting, response inspection and explicit application to channel-specific EQ stages. Existing racks migrate without changing their sound. R2 remains in progress; the report separates completed work from remaining target, FIR and output-binding features.
...
**Implemented serial slice:** [R1e](r1e-serial-rack-and-backups.md) provides 16 ordered node instances, wet/dry/bypass/duplicate controls, immutable prepared schedules, bounded graph transitions, stable effect history, explicit Legacy/recommended templates and processing through normal/crossfade/Studio global paths. Separate saved subchains, parallel/channel routing, full long-IR tail preservation and physical performance acceptance remain open.
...
**Implemented slices:** [R1a](r1a-processing-presets.md) provides local versioned snapshots, atomic settings restore, immutable IR copies and named CRUD. [R1b](r1b-portability-measurements-and-engine.md) adds portable single-preset bundles. [R1e](r1e-serial-rack-and-backups.md) completes graph-aware preset snapshots/migration and full app-backup IR inclusion, validated remapping and recoverable restore rollback. Asset garbage collection and future measurement-asset types remain extensions. Existing automatic device-correction rules are preserved; PRESET-02/03 are not completed by these slices.
...
**Implemented slices:** R1e provides the dedicated 64-band EQ editor and a conservative **64 total PEQ bands across a 16-stage rack**. [R2a](r2a-eq-import-response-and-phone-validation.md) adds strict PK/LSC/HSC file/paste import, parametric-only text export and calculated magnitude, unwrapped phase and group delay for a dedicated EQ stage at the decoder rate. Plots include that stage's graphic EQ and wet/bypass, with explicit scope and Nyquist limits. [R2b](r2b-measurement-tuning-and-phone-validation.md) adds Both/Left/Right routing for dedicated EQ stages and configurable independent measurement fitting within the same total band budget. Separate Gain stages, other stages and hardware are outside the stage-response plot. Per-instance expansion beyond the total budget, additional filter families and per-band bypass remain open.
...
**Implemented R2b slice:** local project CRUD and portable JSON; raw Hz/dB/optional-degree CSV/TXT/FRD and REW numeric-export subset; separate L/R inputs and custom imported or relative-flat target; rig/source/notes; design sample rate, shared normalization, smoothing and band/boost/cut/Q/frequency limits; cancellable peak-filter fitting; measured/target/fitted/predicted plots, saved desired correction values and fit errors; estimated common preamp; explicit atomic application before a final limiter when present; complete project inclusion in app backups. Raw phase is retained, not corrected. The existing online Squig picker and legacy fitter are unchanged. Branded target catalogs, dedicated Squig/AutoEQ/Wavelet/Poweramp adapters, target shaping, delay alignment, fit undo/A-B and reference-optimizer comparisons remain open.

Primary files: [AutoEqRepository][autoeq], [Squig client][squig-client], [Squig repository][squig-repo], [fitter][fitter], [EQ UI][eq-ui], [device preset notes][device-presets]; new `data/tuning/` and `TuningProjectsScreen.kt`.
...
| **R1 — High-precision serial processing** | PRESET-01; ENG-01/02; serial part of ENG-03; essential OBS taps. **Implemented slices R1a–R1e: saved/portable states, meters, binary64 effects, decoder/output integration, persisted serial rack/editor and graph-aware app backups. Compatibility and Studio global processing now have one final PCM16 conversion; Studio per-clip DSP retains its earlier PCM16 boundary. Physical acceptance, additional outputs, full tail transitions and saved subchains remain.** | Full presets migrate, no intermediate PCM16 loss in the new processed path, declared precision matches active nodes, baseline playback regressions pass. |
| **R2 — Everyday tuning depth** | EQ; TUNE import/fitting; basic FIR manager; PRESET-02; OBS headroom/meter basics. **R1e adds 64-band rack storage/editing/processing; [R2a](r2a-eq-import-response-and-phone-validation.md) adds filter interchange and calculated stage response; [R2b](r2b-measurement-tuning-and-phone-validation.md) adds saved measurement projects, configurable fitting and independent L/R correction. Expanded filter families, target shaping/catalogs, IR management and reliable output bindings remain.** | 64-band editing/import, measurement projects, accurate response plots, separate L/R correction and reliable output bindings. |
...
- [x] Write the [ENG/PRESET contract](r1b-precision-engine-contract.md) and [measure the precision-path prototype](r1b-portability-measurements-and-engine.md) before expanding the number of effects.

...
class SettingsStore(private val context: Context) {
    ...
    val processingSettings: Flow<ProcessingSettings> = context.dataStore.data.map { p ->
        ProcessingSettings(readAudioPrefs(p), readPlaybackPrefs(p))
    }.distinctUntilChanged()

    val processingSnapshot: Flow<ProcessingSnapshot> = context.dataStore.data.map { p ->
        ProcessingSnapshot(readAudioPrefs(p), ProcessingPlaybackPrefs.from(readPlaybackPrefs(p)),
            p[Keys.AUTOEQ_PROFILE].orEmpty())
    }.distinctUntilChanged()
        ...
        runCatching {
            ...
            context.dataStore.edit { p ->
                ...
                if (audio.dspConvIrPath.isNotBlank()) {
                    ...
                    } else require(!audio.dspConvEnabled) { "The selected impulse response is missing. Select it again before saving." }
                } else require(!audio.dspConvEnabled) { "Select an impulse response before saving enabled convolution." }
                val preset = ProcessingPreset(UUID.randomUUID().toString(), title,
                    createdAtMs = System.currentTimeMillis(), audio = audio,
                    playback = ProcessingPlaybackPrefs.from(readPlaybackPrefs(p)),
                    activeEqProfile = p[Keys.AUTOEQ_PROFILE].orEmpty(), irSha256 = hash)
    ...
    suspend fun applyProcessingPreset(id: String): Result<ProcessingPresetApplyResult> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            context.dataStore.edit { p ->
                ...
                val oldFloatPath = old.bitPerfectUsb || (old.preferHighRes && readAudioPrefs(p).dspMode != DspMode.CUSTOM)
                val newFloatPath = preset.playback.bitPerfectUsb ||
                    (preset.playback.preferHighRes && preset.audio.dspMode != DspMode.CUSTOM)
                val restart = old.preferHighRes != preset.playback.preferHighRes ||
                    old.bitPerfectUsb != preset.playback.bitPerfectUsb || oldFloatPath != newFloatPath
                writeProcessingAudio(p, preset.audio)

...
class PlaybackService : MediaLibraryService() {
    ...
    private class SinkEvidence(val mono: MonoAudioProcessor? = null, val dsp: AuroraDspProcessor? = null, val convolution: ConvolutionProcessor? = null) {
    ...
    private val extraChains = mutableListOf<Triple<MonoAudioProcessor, AuroraDspProcessor, ConvolutionProcessor>>()
    ...
    override fun onCreate() {
        ...
        val initialEvidence = SinkEvidence(monoProcessor, auroraDsp, convolver)
        ...
        val renderersFactory = object : DefaultRenderersFactory(this) {
            override fun buildAudioSink(
                ...
                val base = DefaultAudioSink.Builder(context)
                    .setAudioProcessors(arrayOf(monoProcessor, auroraDsp, convolver, initialEvidence.afterMeterProcessor))
                    ...
                    .build()
                return TappingAudioSink(base, container.visualizer, initialEvidence.beforeMeter) { initialEvidence.decoded = it }
            }
        }
    ...
    private fun applyAudioEngine() {
        ...
        if (ap.dspConvIrPath != lastIrPath) {
            ...
            scope.launch {
                ...
                convolver.setImpulse(ir, makeupDb)
                extraChains.forEach { it.third.setImpulse(ir, makeupDb) }
        ...
        extraChains.forEach { (mono, dsp, conv) ->
            mono.enabled = monoProcessor.enabled
            dsp.update(params)
            dsp.enabled = auroraDsp.enabled
            conv.enabled = convolver.enabled
            conv.setMakeup(ap.dspConvMakeupDb)
        }
        updateSignalPath()
    }
    ...
    private fun updateSignalPath() {
        ...
        if (mixPlayer != null && active === mixPlayer) {
            ...
            container.signalPath.value = buildSignalPath(SignalPathFacts(kind = if (active.playbackState == Player.STATE_IDLE ||
        ...
        val floatPath = usesFloatPcmPath(useFloatOut, decoded)
        ...
        val customDspActive = processors && state?.dsp?.enabled == true && state.dsp.processingActive
        val monoLocation = monoProcessingLocation(monoAudioPref, customDspActive,
            processors && state?.mono?.enabled == true && state.mono.isActive)
        ...
        node(ap?.dspMode == DspMode.CUSTOM, customDspActive,
            "Custom Aurora DSP (binary64 coefficients, arithmetic and state; PCM16 input/output)")
        node(ap?.dspConvEnabled == true, processors && state?.convolution?.processingActive == true,
            "Convolution (binary64 FFT and state; PCM16 input/output)")
        if (processors && ap?.dspConvEnabled == true) {
            when (state?.convolution?.preparationState) {
                ConvolutionPreparationState.PREPARING -> unknown += "Convolution is preparing its impulse response; new input waits until preparation completes"
                ConvolutionPreparationState.FAILED -> unknown += "Convolution is unavailable; dry audio is used. " +
                    (state.convolution.preparationFailure ?: "Impulse response could not be prepared")
                ConvolutionPreparationState.IDLE -> unknown += impulseLoadFailure
        ...
        requestBitPerfect(mixerDevice, track, !usb && !bitPerfect && useFloatOut && mixerDevice != null && isUsb(mixerDevice.type))
        ...
        val desiredFloat = exclusiveUsbPref || (preferHighResPref && ap?.dspMode != DspMode.CUSTOM)
        ...
        )).copy(measurements = if (!usb && !bitPerfect && raw != null && state != null)
            com.aurora.music.data.AudioMeasurements(state.beforeMeter.snapshot(),
                if (processors && state.afterMeterProcessor.isActive) state.afterMeter.snapshot() else null,
                player.isPlaying, processors && state.afterMeterProcessor.isActive, xfadeActive)
            else null)
    }
    ...
    private fun ensureFadePlayer(): ExoPlayer {
        ...
        val mono = MonoAudioProcessor()
        val dsp = AuroraDspProcessor()
        val conv = ConvolutionProcessor()
        val fadeEvidence = SinkEvidence(mono, dsp, conv)
        extraChains.add(Triple(mono, dsp, conv))
        applyAudioEngine()
        conv.setImpulse(currentImpulse, lastAudioPrefs?.dspConvMakeupDb ?: 0f)
        val factory = object : DefaultRenderersFactory(this) {
            override fun buildAudioSink(context: Context, enableFloatOutput: Boolean, enableAudioTrackPlaybackParams: Boolean): AudioSink =
                TappingAudioSink(DefaultAudioSink.Builder(context)
                    .setAudioProcessors(arrayOf(mono, dsp, conv, fadeEvidence.afterMeterProcessor))
                    .setEnableFloatOutput(useFloatOut)
                    .setEnableAudioTrackPlaybackParams(useFloatOut)
                    .build(), container.visualizer, fadeEvidence.beforeMeter) { fadeEvidence.decoded = it }
        }

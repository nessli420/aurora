...
class MixPlayer(
    ...
    private data class Deck(val player: ExoPlayer, val dsp: AuroraDspProcessor, var clip: MixClip,
        val globalDsp: AuroraDspProcessor, val mono: MonoAudioProcessor, val convolver: ConvolutionProcessor, var started: Boolean = false, var bassCut: Float = 0f)
    ...
    private fun createDeck(clip: MixClip) {
            ...
            val globalDsp = AuroraDspProcessor()
            val mono = MonoAudioProcessor()
            val convolver = ConvolutionProcessor()
            val factory = object : DefaultRenderersFactory(context) {
                override fun buildAudioSink(context: Context, enableFloatOutput: Boolean, enableAudioTrackPlaybackParams: Boolean): AudioSink =
                    DefaultAudioSink.Builder(context).setEnableFloatOutput(false)
                        .setAudioProcessors(arrayOf(MixStereoProcessor(), dsp, mono, globalDsp, convolver)).build()
            }
            ...
            val deck = Deck(p, dsp, clip, globalDsp, mono, convolver)
            decks.add(deck)
            configure(deck)
    ...
    private fun configureGlobal(deck: Deck, config: MixAudioConfig, changedImpulse: Boolean) {
        deck.globalDsp.update(config.params)
        deck.globalDsp.enabled = config.mode == DspMode.CUSTOM
        deck.mono.enabled = config.mono && config.mode != DspMode.CUSTOM
        deck.convolver.enabled = config.convolution
        deck.convolver.setMakeup(config.convolutionGain)
        if (changedImpulse) deck.convolver.setImpulse(config.impulse, config.convolutionGain)
    }

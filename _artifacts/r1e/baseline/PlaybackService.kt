...
class PlaybackService : MediaLibraryService() {
    ...
    private fun applyAudioEngine() {
        ...
        if (ap.dspConvIrPath != lastIrPath) {
            ...
            scope.launch {
                ...
                precisionChains.forEach { it.setImpulse(ir, makeupDb) }
    ...
    private fun updateSignalPath() {
        ...
        val customDspActive = ap?.dspMode == DspMode.CUSTOM &&
            (if (precise) precisionChain?.processingActive == true else processors && state?.dsp?.enabled == true && state.dsp.processingActive)
        val monoLocation = monoProcessingLocation(monoAudioPref, customDspActive,
            if (precise) monoAudioPref && precisionChain?.processingActive == true
            else processors && state?.mono?.enabled == true && state.mono.isActive)
        ...
        node(ap?.dspConvEnabled == true, if (precise) precisionChain?.convolutionProcessingActive == true else processors && state?.convolution?.processingActive == true,
        ...
        if ((processors || precise) && ap?.dspConvEnabled == true) {
            when (if (precise) precisionChain?.preparationState else state?.convolution?.preparationState) {
                ...
                ConvolutionPreparationState.FAILED -> unknown += "Convolution is unavailable; dry audio is used. " +
                    ((if (precise) precisionChain?.preparationFailure else state?.convolution?.preparationFailure) ?: "Impulse response could not be prepared")

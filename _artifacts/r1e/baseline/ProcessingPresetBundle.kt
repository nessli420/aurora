...
object ProcessingPresetBundle {
    ...
    private const val MAX_MANIFEST_BYTES = 128L * 1024
    ...
    fun write(preset: ProcessingPreset, impulseResponse: File?, output: OutputStream) {
        ...
        require(!portable.audio.dspConvEnabled || impulseResponse != null) {
    ...
    fun read(input: InputStream, temporaryRoot: File): ImportedProcessingPresetBundle {
        ...
        try {
            ...
            ZipFile(archive).use { zip ->
                ...
                require(!preset.audio.dspConvEnabled || hasIr) { "Enabled convolution requires an included impulse response." }
                if (hasIr) {
                    val digest = MessageDigest.getInstance("SHA-256")
    ...
    private fun parseManifest(json: String): ProcessingPreset {
        ...
        require(version.isJsonPrimitive && version.asJsonPrimitive.isNumber && version.asDouble == VERSION.toDouble()) {
        ...
        require(p.keySet() == presetKeys) { "Incomplete or unsupported preset settings." }
    ...
    private fun readJson(reader: JsonReader, depth: Int, count: IntArray): JsonElement {
        require(depth <= 16 && ++count[0] <= 4096) { "Preset metadata is too complex." }
    ...
    private fun validateImpulseResponse(file: File) {

...
class PlaybackService : MediaLibraryService() {
    ...
    private fun applyAudioEngine() {
        ...
        if (ap.dspConvIrPath != lastIrPath) {
            ...
            scope.launch {
                val ir = kotlinx.coroutines.withContext(Dispatchers.IO) {
                    ap.dspConvIrPath.takeIf { it.isNotBlank() }
                ...
                extraChains.forEach { it.third.setImpulse(ir, ap.dspConvMakeupDb) }
    ...
    private fun updateSignalPath() {
        ...
        val customDspActive = processors && state?.dsp?.enabled == true && state.dsp.isActive
        ...
        node(ap?.dspMode == DspMode.CUSTOM, customDspActive,
            "Custom Aurora DSP (float math, PCM16 between stages)")
        node(ap?.dspConvEnabled == true, processors && state?.convolution?.processingActive == true, "Convolution")

...
fun buildSignalPath(f: SignalPathFacts): SignalPath {
    ...
    if (f.kind == PlaybackPathKind.MIX) return SignalPath(
        ...
        processing = SignalStage("Processing", "Per-deck stereo conversion → clip DSP → global processing → timeline gains", "Mix player processing schedule; PCM16 transport, float DSP math"),

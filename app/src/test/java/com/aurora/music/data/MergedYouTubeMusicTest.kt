...
class MergedYouTubeMusicTest {
    ...
    @Test fun catalogueSearchDoesNotQueryServersUntilPlaybackAndKeepsLikesOnYoutube() = runBlocking {
        ...
        assertEquals(PlaybackSourceIdentity.fromSession(server, hosted.albumId), candidate.playbackSource?.copy(songId = null))
        assertEquals(hosted.id, stripMergeNamespace(candidate.playbackSource?.songId.orEmpty()))

...
internal data class ArtworkRequest(
    ...
    val key: String get() = MessageDigest.getInstance("SHA-256").digest(
        listOf(artist, album, title, original, duration?.toString()).joinToString("\u0000").toByteArray()
...
object ArtworkUrls {
    ...
    internal fun decode(uri: String): ArtworkRequest? = runCatching {
        ...
        if (!request.original.isNullOrBlank() && !isMediaStoreArt(request.original)) return null
